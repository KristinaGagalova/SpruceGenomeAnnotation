...
genome=_RePlaCe_ #genome sequence (fasta file or fasta embeded in GFF3 file)
...
altest=/path/to/alt/org/evidence #EST/cDNA sequence file in fasta format from an alternate organism
est_gff= #aligned ESTs or mRNA-seq from an external GFF3 file
altest_gff= #aligned ESTs from a closly relate species in GFF3 format

#-----Protein Homology Evidence (for best results provide a file for at least one)
protein=/path/to/evidence  #protein sequence file in fasta format (i.e. from mutiple oransisms)
protein_gff=  #aligned protein homology evidence from an external GFF3 file

#-----Repeat Masking (leave values blank to skip repeat masking)
model_org= #all #select a model organism for RepBase masking in RepeatMasker
rmlib=/path/to/library #provide an organism specific repeat library in fasta format for RepeatMasker
...
#-----Gene Prediction
snaphmm=/path/to/snaphmm #SNAP HMM file
gmhmm=/path/to/gmhmm #GeneMark HMM file
augustus_species=spruceModel #Augustus gene prediction species model
...
pred_flank=500 #flank for extending evidence clusters sent to gene predictors
